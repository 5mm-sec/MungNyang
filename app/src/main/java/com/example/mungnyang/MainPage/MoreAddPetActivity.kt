...
class MoreAddPetActivity : AppCompatActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.savetextview.setOnClickListener {
            ...
            if (petName.isEmpty() || birthday.isEmpty() || gender.isEmpty() || kind.isEmpty() || kindEdit.isEmpty() || weight.isEmpty()){
                Toast.makeText(this@MoreAddPetActivity, "필수 항목을 모두 작성 해 주세요!", Toast.LENGTH_SHORT).show()
                    ...
                    override fun onResponse(call: Call<ResponsePetDTO>, response: Response<ResponsePetDTO>) {
                        ...
                        if (responseDto != null) {
                            ...
                            if (responseDto.response) {
                                Toast.makeText(this@MoreAddPetActivity, "등록 성공!", Toast.LENGTH_SHORT).show()

                                ProfileManagementOBJ.moreAddPetRefreshFrom(accountEmail.toString(), petName) { petID ->
                                    val newProfile = Profile(petID, URL, petName, accountEmail.toString())
                                    Log.d("추가 할때 petID", petID)
                                    ProfileManagementOBJ.addProfile(newProfile) // 필드 추가 성공시 갱신
                                }
                                ...
                                Toast.makeText(this@MoreAddPetActivity, "등록 불가능!", Toast.LENGTH_SHORT)
                                    .show()
                            }
                        } else {
                            Toast.makeText(this@MoreAddPetActivity, "응답이 없습니다.", Toast.LENGTH_SHORT)
                                .show()
                        }
                    }

                    override fun onFailure(call: Call<ResponsePetDTO>, t: Throwable) {
                        Toast.makeText(this@MoreAddPetActivity, "등록 불가능 네트워크 에러!", Toast.LENGTH_SHORT)
                            .show();
                    }
                })
