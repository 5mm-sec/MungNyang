        ...
        if (binding.detailEdittext.text.length < 4 || binding.detailEdittext.text.length > 50) {
            ...
            binding.symptomCheckAddtextview.setOnClickListener {
                ...
                for (number in selectedImageNumbers) {
                    ...
                    val textId = resources.getIdentifier("symptom_check_text$number", "id", packageName)
                ...
                val addSymptomDTO = SymptomDTO(selectedDate,
                    petID?.toInt(),
                    symptomList )
                    ...
                    override fun onResponse(call: Call<ResponseSymptomDTO>, response: Response<ResponseSymptomDTO>) {
                        ...
                        if (responseDto != null) {
                            ...
                            if (responseDto.response) {
                                ...
                                Toast.makeText(this@SymptomCheckActivity, "저장 완료!", Toast.LENGTH_SHORT).show()
                                ...
                                Log.d("메인 이상 증상 serializedEventDates", serializedEventDates.toString())

                                intent.putStringArrayListExtra("eventDates", serializedEventDates) // eventDates는 데이터의 예시로, 실제 데이터로 대체해야 합니다.
                                intent.putStringArrayListExtra("symptomList", ArrayList(symptomList))
                                setResult(RESULT_OK, intent)
                                finish()

                            } else {
                                Toast.makeText(this@SymptomCheckActivity, "저장 불가능!", Toast.LENGTH_SHORT)
                                    .show()
                            }
                        } else {
                            Toast.makeText(this@SymptomCheckActivity, "응답이 없습니다.", Toast.LENGTH_SHORT)
                                .show()
                        }
                    }

                    override fun onFailure(call: Call<ResponseSymptomDTO>, t: Throwable) {
                        Toast.makeText(this@SymptomCheckActivity, "등록 불가능 네트워크 에러!", Toast.LENGTH_SHORT)
                            .show();
                    }
                })
            }
